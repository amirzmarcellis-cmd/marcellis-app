...
export default function JobDetails() {
  ...
  return <div className="space-y-4 md:space-y-6 p-4 md:p-6 max-w-full overflow-hidden">
                              ...
                              <CardContent className="p-3 md:p-4">
                                ...
                                return <Button key={latestContact.callid} variant="outline" size="sm" asChild className="flex-1 min-w-0 text-xs md:text-sm">
                                    ...
                                    <Button 
                                      variant="outline" 
                                      size="sm" 
                                      onClick={() => setSelectedCandidateRecord(mainCandidate)}
      ...
      setIsEditDialogOpen(false);
        ...
        {/* Call Log Details Dialog */}
        <Dialog open={!!selectedCandidateRecord} onOpenChange={() => setSelectedCandidateRecord(null)}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Jobs_CVs Record Details</DialogTitle>
              <DialogDescription>
                Complete information for {selectedCandidateRecord?.candidate_name || 'candidate'}
              </DialogDescription>
            </DialogHeader>
            ...
            {selectedCandidateRecord && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="grid grid-cols-2 gap-2">
                      ...
                      <span className="font-mono">{selectedCandidateRecord.recordid}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Job ID:</span>
                      <span className="font-mono">{selectedCandidateRecord.job_id}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Itris Job ID:</span>
                      <span className="font-mono">{selectedCandidateRecord.itris_job_id || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">User ID:</span>
                      <span className="font-mono">{selectedCandidateRecord.user_id || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Recruiter ID:</span>
                      <span className="font-mono">{selectedCandidateRecord.recruiter_id || 'N/A'}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Candidate Details */}
                <div className="space-y-3">
                  <h3 className="font-semibold text-sm border-b pb-1">Candidate Details</h3>
                  <div className="space-y-2 text-sm">
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Name:</span>
                      <span>{selectedCandidateRecord.candidate_name || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Email:</span>
                      <span className="break-all">{selectedCandidateRecord.candidate_email || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Phone:</span>
                      <span>{selectedCandidateRecord.candidate_phone_number || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Notice Period:</span>
                      <span>{selectedCandidateRecord.notice_period || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Current Salary:</span>
                      <span>{selectedCandidateRecord.current_salary || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Salary Expectations:</span>
                      <span>{selectedCandidateRecord.salary_expectations || 'N/A'}</span>
                    </div>
                  </div>
                </div>
                ...
                {/* Scoring Information */}
                <div className="space-y-3">
                  <h3 className="font-semibold text-sm border-b pb-1">Scoring</h3>
                  <div className="space-y-2 text-sm">
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">CV Score:</span>
                      <span className="font-medium">{selectedCandidateRecord.cv_score || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">After Call Score:</span>
                      <span className="font-medium">{selectedCandidateRecord.after_call_score || 'N/A'}</span>
                    </div>
                  </div>
                  {selectedCandidateRecord.cv_score_reason && (
                    <div className="space-y-1">
                      <span className="text-muted-foreground text-sm">CV Score Reason:</span>
                      <p className="text-sm p-2 bg-muted/50 rounded-md">{selectedCandidateRecord.cv_score_reason}</p>
                    </div>
                  )}
                  {selectedCandidateRecord.after_call_reason && (
                    <div className="space-y-1">
                      <span className="text-muted-foreground text-sm">After Call Reason:</span>
                      <p className="text-sm p-2 bg-muted/50 rounded-md">{selectedCandidateRecord.after_call_reason}</p>
                    </div>
                  )}
                </div>
                ...
                {/* Call Information */}
                <div className="space-y-3">
                  <h3 className="font-semibold text-sm border-b pb-1">Call Information</h3>
                  <div className="space-y-2 text-sm">
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Contacted:</span>
                      <span>{selectedCandidateRecord.contacted || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Call Count:</span>
                      <span>{selectedCandidateRecord.callcount || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Duration:</span>
                      <span>{selectedCandidateRecord.duration || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Last Call Time:</span>
                      <span>{selectedCandidateRecord.lastcalltime || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Recording:</span>
                      <span>{selectedCandidateRecord.recording ? 'Available' : 'N/A'}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-3 md:col-span-2">
                  <h3 className="font-semibold text-sm border-b pb-1">Interview & Assessment</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    {selectedCandidateRecord.two_questions_of_interview && (
                      <div className="space-y-1">
                        <span className="text-muted-foreground text-sm">Interview Questions:</span>
                        <p className="text-sm p-2 bg-muted/50 rounded-md">{selectedCandidateRecord.two_questions_of_interview}</p>
                      </div>
                    )}
                    {selectedCandidateRecord.after_call_pros && (
                      <div className="space-y-1">
                        <span className="text-muted-foreground text-sm">Pros:</span>
                        <p className="text-sm p-2 bg-green-50 dark:bg-green-950/20 rounded-md">{selectedCandidateRecord.after_call_pros}</p>
                      </div>
                    )}
                    {selectedCandidateRecord.after_call_cons && (
                      <div className="space-y-1">
                        <span className="text-muted-foreground text-sm">Cons:</span>
                        <p className="text-sm p-2 bg-red-50 dark:bg-red-950/20 rounded-md">{selectedCandidateRecord.after_call_cons}</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {/* Transcript */}
                {selectedCandidateRecord.transcript && (
                  <div className="space-y-3 md:col-span-2">
                    <h3 className="font-semibold text-sm border-b pb-1">Call Transcript</h3>
                    <div className="max-h-48 overflow-y-auto p-3 bg-muted/30 rounded-md">
                      <p className="text-sm whitespace-pre-wrap">{selectedCandidateRecord.transcript}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="space-y-3 md:col-span-2">
                  <h3 className="font-semibold text-sm border-b pb-1">Timeline</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Longlisted At:</span>
                      <span>{selectedCandidateRecord.longlisted_at || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Shortlisted At:</span>
                      <span>{selectedCandidateRecord.shortlisted_at || 'N/A'}</span>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                      <span className="text-muted-foreground">Notes Updated At:</span>
                      <span>{selectedCandidateRecord.notes_updated_at || 'N/A'}</span>
                    </div>
                  </div>
                </div>

                {/* Notes */}
                {selectedCandidateRecord.notes && (
                  <div className="space-y-3 md:col-span-2">
                    <h3 className="font-semibold text-sm border-b pb-1">
                      Notes {selectedCandidateRecord.notes_updated_by && (
                        <span className="text-muted-foreground font-normal">(Updated by: {selectedCandidateRecord.notes_updated_by})</span>
                      )}
                    </h3>
                    <div className="p-3 bg-muted/30 rounded-md">
                      <p className="text-sm whitespace-pre-wrap">{selectedCandidateRecord.notes}</p>
                    </div>
                  </div>
                )}
              </div>
