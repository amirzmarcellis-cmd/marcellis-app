...
CREATE OR REPLACE FUNCTION disable_auto_dial_at_threshold()
RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
  ...
  IF current_auto_dial = TRUE THEN
    ...
    IF shortlisted_count >= 6 THEN
      ...
      -- Log for debugging
      RAISE NOTICE 'Auto-dial disabled for job % - reached % shortlisted candidates', NEW.job_id, shortlisted_count;
    END IF;
  END IF;
...
CREATE TRIGGER trigger_disable_auto_dial
  ...
  ON "Jobs_CVs"
  FOR EACH ROW
  EXECUTE FUNCTION disable_auto_dial_at_threshold();